...
export default {
  ...
  computed: {
    dataCollection() {
      return {
        labels: sliceAndFill(this.totalLabels, this.offset + 500, this.plotNum),
        datasets: [
          {
            label: 'Data One',
            backgroundColor: '#f87979',
            data: sliceAndFill(this.totalData, this.offset, this.plotNum)
          }
        ]
      }
    }
  ...
  methods: {
    ...
    fillData() {
      ...
      this.totalData = Array.from({ length: 200 }, () => getRandomInt())
    }
  }
}
</script>
